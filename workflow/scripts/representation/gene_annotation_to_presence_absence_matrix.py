...
def create_strain_df(strain_id, genes_reference, genes_strain):
    ...
    df_strain_present_genes = pd.DataFrame(
        columns=genes_strain,
        index=[strain_id],
        data=np.ones(len(genes_strain))[np.newaxis, :].astype(int),
    )
    genes_reference_only = genes_reference.difference(genes_strain)

    df_genes_reference_only = pd.DataFrame(
        columns=genes_reference_only,
        index=[strain_id],
        data=np.zeros(len(genes_reference_only))[np.newaxis, :].astype(int),
    )

    df_strain = pd.concat([df_strain_present_genes, df_genes_reference_only], axis=1)
...
genes_reference = set(
    reference_data["gene"][np.logical_not(reference_data["gene"].isnull())]
)
...
for file in file_list:
    ...
    genes_strain = set(
        strain_data["gene"][np.logical_not(strain_data["gene"].isnull())]
    )
