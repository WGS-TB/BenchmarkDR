...
for path, currentDirectory, files in os.walk(
    "/mnt/c/Users/Fernando/Documents/Project_2/output/mtb/prediction/results"
):
    for file in files:
    ...
    fig = go.Figure(
        data=go.Parcoords(
            ...
            dimensions=list(
                [
                    dict(
                        tickvals=dff["param_solver_ind"],
                        ticktext=["liblinear", "saga"],
                        label="Solver",
                        values=dff["param_solver_ind"],
                    ),
                    dict(
                        range=[min(dff["param_C_log"]), max(dff["param_C_log"])],
                        label="log10(C)",
                        values=dff["param_C_log"],
                    ),
                    dict(
                        range=[
                            min(dff["param_max_iter_log"]),
                            max(dff["param_max_iter_log"]),
                        ],
                        label="log10(Max iter)",
                        values=dff["param_max_iter_log"],
                    ),
                    dict(
                        range=[0, 1],
                        label="Balanced accuracy",
                        values=dff["mean_test_balanced_accuracy"],
                    ),
